...
const CloudSync = {
  ...
  // Called from background.js's chrome.runtime.onMessageExternal listener when
  // the website (a page matched in manifest.json's externally_connectable)
  // pushes a Firebase session it just created via its own Google sign-in. The
  // session shape — {uid, idToken, refreshToken, expiresAt} — is identical to
  // what signInWithGoogleToken()/refreshSession() already produce above, so
  // every existing pull/push/refresh code path works on it unchanged.
  async adoptExternalSession(session, profile) {
    if (!session || !session.idToken || !session.refreshToken) {
      throw new Error("Incomplete session pushed from website");
    }
    await saveSession(session);
    if (profile) await self.Store.setUser(profile);
    try {
      await pullFromCloud({
        force: !1
      });
    } catch (e) {
      console.warn("[NexSion] Post-adopt cloud pull failed:", e);
    }
  },
  // Called when the website signs out. Deliberately does NOT wipe local
  // boards/links the way the newtab UI's own "Sign Out" button does (that one
  // shows a confirm dialog first) — an external signal shouldn't be able to
  // delete local data without the person ever seeing that warning. It just
  // ends the cloud session, matching "signed out of sync" rather than
  // "erase this device".
  async signOutExternal() {
    clearTimeout(pushTimer);
    pushTimer = null;
    await clearSession();
    await self.Store.setUser(null);
  },
